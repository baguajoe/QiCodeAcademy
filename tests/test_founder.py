...
EXPECTED_SHORT_FIRST = (
    ...
    "Tai Chi, Baguazhang, and Xing Yi. A Boston native, he currently teaches at Grove Hall Senior Center, the Parkway "
    "Community YMCA in West Roxbury, Codman Square Library, and in Boston City Parks, where he has taught since 2014.")
EXPECTED_SHORT_SECOND = (
    "Joseph is also a 200-hour certified yoga instructor and holds a massage therapy diploma from a 10-month, 650-hour "
    "program at the New England School of Therapeutics, along with myofascial therapy and Asian bodywork "
    "certifications. He is the founder of Eye Forge Studios LLC and earned a Full-Stack Web Developer certification "
    "from 4Geeks Academy in 2024.")
...
def test_founder_text_is_verbatim():
    ...
    assert FOUNDER_SHORT_BIO.split("\n\n")[1].startswith(EXPECTED_SHORT_SECOND)
    ...
    assert ("## Yoga, massage, and bodywork\n\nJoseph earned his 200-hour yoga teacher certification from 33 Degree "
            "Yoga in 2020. He then completed a 10-month, 650-hour massage therapy program at the New England School of "
            "Therapeutics under Richmond Dickson, earning his diploma in 2021. He holds a myofascial therapy "
            "certification and an Asian Bodywork Therapy certification from Master Tak Wong.") in FOUNDER_FULL_BIO
    assert "He is the founder of Eye Forge Studios LLC, the author of the comic book Iron Dragon" in FOUNDER_FULL_BIO
    for phrase in ("Gin Soon Tai Chi", "Grandmaster Gin Soon Chu", "Grandmaster Yang Sau Chung", "Yang Cheng Fu",
                   "Arthur Goodridge", "T.T. Liang", "650-hour", "Richmond Dickson", "Eye Forge Studios",
                   "Iron Dragon", "Maya 3D"):
        assert phrase in FOUNDER_FULL_BIO
    headings = re.findall(r"^## (.+)$", FOUNDER_FULL_BIO, re.M)
    assert headings == ["Lineage and training", "Yoga, massage, and bodywork", "Technology, media, and creative work"]
...
def test_previous_version_upgrades_when_unedited(db):
    """A database seeded with the previous founder text/locations upgrades on the next `flask seed`."""
    from api.commands import LEGACY_TEACHING_LOCATIONS, TEACHING_LOCATIONS
    from api.founder_content import fingerprint
    seed_settings()
    seed_founder()
    db.session.commit()
    # Every stored legacy fingerprint must be distinct from the current text.
    assert fingerprint(FOUNDER_SHORT_BIO) not in LEGACY_FINGERPRINTS["short_bio"]
    assert fingerprint(FOUNDER_FULL_BIO) not in LEGACY_FINGERPRINTS["full_bio"]
    assert fingerprint(FOUNDER_CREDENTIALS) not in LEGACY_FINGERPRINTS["credentials"]
    loc = db.session.query(SiteSetting).filter_by(key="teaching_locations").one()
    loc.value = next(iter(LEGACY_TEACHING_LOCATIONS))
    db.session.commit()
